...
def test():
    try:
        service = NLPService()
        schema_str = service.generate_schema("500L CIP tank with spray ball, vent filter, outlet connected to lobe pump via valve")
        print("Raw JSON:", schema_str)
        raw_schema = SchemaBuilder.build_from_json(schema_str)
        print("\nParsed Schema Components:", raw_schema.components)
        print("Parsed Schema Connections:", raw_schema.connections)
